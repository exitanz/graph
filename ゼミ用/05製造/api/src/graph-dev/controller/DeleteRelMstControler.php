<?php
require_once dirname(__FILE__) . '/../request/DeleteRelMstRequest.php';
...
try {
    ...
    // リクエストの値を格納
    $deleteRelMstRequest = new DeleteRelMstRequest($REQUEST['rel_mst_id'], $REQUEST['user_id']);

    // バリデーションチェック
    if ($deleteRelMstRequest->validation()) {
        // バリデーション違反
        http_response_code(400);
        $resultCode = ResultCode::CODE101;
        $msg = $deleteRelMstRequest->getErrorMsg();
        throw new Exception();
    }
    ...
    try {
        ...
        $relMstService->deleteRelMst($deleteRelMstRequest->getRelMstId(), $deleteRelMstRequest->getUserId());
