<?php
require_once dirname(__FILE__) . '/../request/DeleteRelRequest.php';
...
try {
    ...
    // リクエストの値を格納
    $deleteRelRequest = new DeleteRelRequest($REQUEST['rel_id'], $REQUEST['user_id']);

    // バリデーションチェック
    if ($deleteRelRequest->validation()) {
        // バリデーション違反
        http_response_code(400);
        $resultCode = ResultCode::CODE101;
        $msg = $deleteRelRequest->getErrorMsg();
        throw new Exception();
    }
    ...
    try {
        ...
        $relService->deleteRel($deleteRelRequest->getRelId(), $deleteRelRequest->getUserId());
