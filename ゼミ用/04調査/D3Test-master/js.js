...
function SVGzoomed() {
  ...
  //倍率表示
  d3.select("#r_zoom_text").text((d3.event.transform.k * 100).toFixed(0)); //スライダーの横にある表示を更新
  document.getElementById("rng_zoom").value = Math.log2(
...
function UpodateNodeLink() {
  ...
  if (nodegroup == "") {
    ...
    d3.selectAll("#linkArrow").attr("visibility", function (d) {
      if (
        d.source.group == nodegroup &&
        d.target.group == nodegroup &&
        d.value >= linkvalue
      ) {
        return "visible";
      } else {
