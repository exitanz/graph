...
var datatip2 = d3.select("#datatip2");
...
d3.json("miserables2.json", function (error, graph) {
  ...
  var links = g
    ...
    .on("mouseover", function (d) {
      ...
      datatip.select("#fromto").text(d.source.id + " から " + d.target.id);
      datatip.select("#content").text("詳細:" + d.groupMemo);
    })
  ...
  node
    ...
    .on("mouseover", function (d) {
      ...
      datatip2.select("#contents").text("グループ:" + d.group);
    })
    .on("mousemove", function () {
      datatip2
        .style("left", d3.event.pageX + 20 + "px")
    ...
    .on("mouseout", function () {
      ...
      datatip2.style("z-index", -1).style("opacity", 0);
    });
  //node textの定義
  node
    .append("text")
